import { type QuizApiResponse, type QuizDetailApiResponse, type QuizDetailResponse, type QuizRequest, type QuizResponse } from "../types/Quiz";
...
const getQuizById = async (id: string): Promise<QuizDetailResponse> => {
    try {
        const response = await apiClient.get<QuizDetailApiResponse>(`/api/quizzes/${id}`);
...
const getQuizByLesson = async (lessonId: string): Promise<QuizResponse[]> => {
    try {
        const response = await apiClient.get<QuizApiResponse>(`/api/quizzes/lesson/${lessonId}`);
        if (response.data.code !== 1000 || !response.data.result) {
...
const createQuiz = async (field: QuizRequest): Promise<QuizResponse> => {
    try {
        const response = await apiClient.post<QuizApiResponse>('/api/quizzes', field);
        if (response.data.code !== 1000 || !response.data.result) {
