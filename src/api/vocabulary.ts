...
const getVocabularyByLessonId = async (lessonId: string): Promise<VocabularyResponse[]> => {
    try {
        const response = await apiClient.get<VocabularyApiResponse>(`/api/vocabularies/lesson/${lessonId}`);
        if (response.data.code !== 1000 || !response.data.result) {
            throw new Error('Failed to fetch vocabularies by lesson ID: Invalid response code');
        }
        console.log('Fetched vocabularies by lesson ID:', response.data.result);
        return response.data.result;
    } catch (error: any) {
        console.error('Error fetching vocabularies by lesson ID:', error);
        throw error;
    }
}
